...
const categoryFromIngredient = (name: string): FoodCategory => {
  ...
  if (/huile|riz|pâte|pates|quinoa|graine|bocal|pain|épeautre|epeautre|brioche|blé|ble/.test(lower)) return 'Épicerie sèche';
  ...
  const addFoodsFromManualScan = (rawText: string) => {
    ...
    updateData((current) => {
      const priceRecords: StorePriceRecord[] = foods
        .filter((food) => typeof food.purchasePrice === 'number' || typeof food.pricePerUnit === 'number')
        .map((food) => ({
          id: newId('price'),
          productName: food.name,
          store: food.store,
          price: food.purchasePrice ?? 0,
          quantity: food.quantity,
          unit: food.unit,
          pricePerUnit: food.pricePerUnit ?? (food.purchasePrice && food.quantity ? food.purchasePrice / food.quantity : 0),
      ...
      return {
        ...current,
        foods: [...foods, ...current.foods],
        storePriceRecords: [...priceRecords, ...current.storePriceRecords],
      };
    });
  };

  const updateStoreHours = (storeId: string, day: Weekday, ranges: OpeningTimeRange[]) => {
    updateData((current) => ({
      ...current,
